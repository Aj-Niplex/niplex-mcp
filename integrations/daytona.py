    ...
    def execute_command(self, command: str, ttl_minutes: int = 0) -> str:
        ...
        def write_file(self, file_path: str, content: str) -> str:
            try:
                os.environ["DAYTONA_API_KEY"] = self.api_key
                daytona = Daytona()
                # We assume a temporary sandbox for the write operation
                # In a real scenario, the agent would provide a sandbox_id
                # For now, we create a transient one to perform the write
                params = CreateSandboxFromSnapshotParams(auto_delete_interval=0)
                sandbox = daytona.create(params)
            
                import base64
                encoded_content = base64.b64encode(content.encode("utf-8")).decode("utf-8")
                # Use a shell command to decode and write the file
