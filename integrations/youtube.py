    ...
    def _strip_bloat(self, data):
        ...
        if isinstance(data, dict):
            essential_fields = {
                ...
                'id': None, 'channelId': None, 'title': None, 'description': None,
            }
            filtered = {}
            for key, allowed_values in essential_fields.items():
                if key in data:
                    if allowed_values is None:
                        ...
                        filtered[key] = {k: v for k, v in data[key].items() if k in allowed_values}
            return filtered
        return data

    def search_videos(self, query, max_results=10, order='relevance'):
        params = {'q': query, 'part': 'snippet', 'maxResults': max_results, 'order': order, 'type': 'video'}
        res = self._make_request('/search', params)
        if 'error' in res: return res['error']
        return json.dumps([self._strip_bloat(item) for item in res.get('items', [])], indent=2)

    def get_video_details(self, video_ids):
        params = {'id': ','.join(video_ids), 'part': 'snippet,statistics,contentDetails'}
